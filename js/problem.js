...
async function loadProblem(){
  ...
  if (!id) { el.innerHTML = '<p class="muted">No ID provided.</p>'; return; }
  try {
    const res = await api('getProblem', { id });
    if (!res) { el.innerHTML = '<p class="muted">Problem not found.</p>'; return; }
    el.innerHTML = `
      ...
      <section class="card" style="margin-top:12px">
        ...
        <hr/>
        <div class="solution" style="display:none">${res.solution || '<span class="muted">No solution</span>'}</div>
    ...
    if (token){
      ...
      document.getElementById('edit-btn').addEventListener('click', ()=> location.href = `edit.html?id=${res.id}`);
    }
  } catch(err){
    el.textContent = 'Error loading problem.'; console.error(err);
  }
}
